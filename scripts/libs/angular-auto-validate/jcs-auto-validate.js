            ...
            function (validator, elementUtils) {
                var elementTypesToValidate = ['input', 'textarea', 'select', 'form'],
                    ...
                    shouldValidateElement = function (el, formOptions) {
                        var result = el && el.length > 0;
                        var elementHtml = $("<div />").append(el.clone()).html();
                        var valid = false;
                        if (elementHtml.indexOf("kendo-") >= 0) {
                            if (el.is(":visible")) {
                                valid = true;
                            }
                            var parent = el.parents(".k-widget:first");
                            if (parent.length > 0) {
                                valid = parent.is(":visible");
                            }
                        } else {
                            valid = (elementIsVisible(el) || formOptions.validateNonVisibleControls) &&
                ...
                return {
                    ...
                    resetForm: resetForm,
                    shouldValidateElement: shouldValidateElement
                };
            }
        ]);
}(angular));
